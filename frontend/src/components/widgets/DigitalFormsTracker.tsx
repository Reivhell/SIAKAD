...
interface EFormRequest {
  ...
  studentName: string;
  studentNim: string;
  type: 'Surat Keterangan Aktif Kuliah' | 'Legalisir Ijazah Resmi' | 'Permohonan Cuti Akademik' | 'Permohonan Pindah Kelas' | 'Permohonan Pengunduran Diri' | 'Permohonan Transkrip Resmi';
  reason: string;
  submittedAt: string;
...
export function DigitalFormsTracker({ role, isAlumni = false }: { role: string; isAlumni?: boolean }) {
  ...
  const [formType, setFormType] = useState<EFormRequest['type']>('Surat Keterangan Aktif Kuliah');
          ...
          <div className="lg:col-span-4 bg-slate-50 dark:bg-slate-950/40 p-5 rounded-2xl border border-slate-200/50 dark:border-slate-800/80 space-y-4">
            ...
            <form onSubmit={handleSubmitRequest} className="space-y-4 text-xs font-semibold">
              <div className="space-y-1.5">
                ...
                <select
                  value={formType}
                  onChange={(e) => setFormType(e.target.value as EFormRequest['type'])}
