...
export class AuthController {
  ...
  async secureLogin(@Req() req: express.Request, @Res({ passthrough: true }) res: express.Response, @Body() body: any) {
    ...
    try {
      ...
      // Token hanya dikirim via HttpOnly cookie, TIDAK di response body.
      // Ini mencegah token dibaca oleh XSS atau disimpan di localStorage secara tidak aman.
      return {
        status: 'success',
  ...
  async resetPasswordRequest(@Req() req: express.Request, @Body() body: any) {
    ...
    // Token dikirim via email (tidak dikembalikan di response untuk keamanan).
    // Di production, integrasikan dengan email service (nodemailer, SendGrid, dll).
    // TODO: Kirim resetToken via email ke user.email
  ...
  async resetPasswordConfirm(@Req() req: express.Request, @Body() body: any) {
    ...
    // Hash new password & update user record before invalidating token.
    // If DB update fails, token remains valid so user can retry.
    ...
    if (!updated) {
      throw new HttpException({ status: 'error', message: 'Gagal memperbarui kata sandi. Silakan coba lagi.' }, HttpStatus.INTERNAL_SERVER_ERROR);
    }
