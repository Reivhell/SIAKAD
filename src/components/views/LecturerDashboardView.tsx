...
export function LecturerDashboardView({ user, activeTab = 'dashboard', onChangeTab, onUserChange }: LecturerDashboardViewProps) {
  ...
  React.useEffect(() => {
    if (user && onUserChange) {
      if (profile.nama !== user.name || profile.email !== user.email || profile.phone !== user.phone || profile.avatar !== user.avatar) {
        onUserChange({
          ...user,
          name: profile.nama,
          email: profile.email,
  ...
  const [jadwal, setJadwal] = useState(initialJadwalMengajar);
  ...
  const kelasCount = kelas.length;
  const totalStudentsCount = students.length;
  const todayClasses = jadwal.filter(j => j.day === 'Senin' || j.day === 'Selasa'); // Simulating today's classes
  const pendingKrsCount = students.filter(s => s.krs.status === 'Pending').length;
